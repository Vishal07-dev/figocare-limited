...
export default function WhereWeWork({ variant = "white" }: WhereWeWorkProps) {
  const isPrimary = variant === "primary";
  
  return (
    <section className={`py-13 ${isPrimary ? "bg-primary" : "bg-white"}`} aria-labelledby="where-we-work-heading">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 id="where-we-work-heading" className={`text-3xl sm:text-4xl font-bold mb-6 ${isPrimary ? "text-white" : "text-gray-900"}`}>
            Our Services
          </h2>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {serviceAreas.map((service, index) => (
            <div
              key={index}
              className="bg-white rounded-2xl p-8 hover:shadow-xl transition-all duration-300 group border border-gray-100 text-center"
            >
